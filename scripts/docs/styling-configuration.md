...
The validation system includes a configuration system for customizing Mermaid graph styling through `risk-map/yaml/mermaid-styles.yaml`. This configuration file controls the visual aspects of the component graph generated by the validation scripts. (The `control` and `risk` sections of this file are unused now that #477 removed the generators that consumed them; they are cleaned up separately.)
