...
**Trigger:** `^risk-map/yaml/(risks|controls|components|personas)\.yaml$`
(`pass_filenames: true`)
...
- Versioned framework + no delimiter (`@` or `:`) → skip (legacy unpinned
  form; migration is #343).
- Delimiter present: attempt split + compose round-trip. Round-trip mismatch
  ...
  error on an unversioned framework (STRIDE legacy spelling) → skip.
...
`validate-all.sh` with the four content paths explicit.
