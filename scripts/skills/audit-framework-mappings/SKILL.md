...
4. The live-verify result (found current / not found / doesn't exist), or, for a framework with no external registry (STRIDE-class), the explicit statement "live-verify not applicable (closed literal set, unversioned, no registry)" rather than omitting the item
