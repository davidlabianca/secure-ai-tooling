...
- `altitude-check/` — checks whether a control/risk/component draft is
  ...
  (ADR-031 D2).
- `audit-framework-mappings/` — audits framework mappings against the
  framework-mappings style guide (version pinning, applicability,
  selectivity) in three modes: a single existing entity, the whole
  corpus, or a candidate value for an entity not yet in the corpus
  (pre-PR authoring), deferring format to the style guide
  (ADR-031 D2, ADR-027).
