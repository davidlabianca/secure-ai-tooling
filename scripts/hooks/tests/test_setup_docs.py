...
def risk_map_setup_content():
    ...
    assert (
        RISK_MAP_SETUP_PATH.exists()
    ), f"Risk map setup doc not found at {RISK_MAP_SETUP_PATH}"
...
def scripts_setup_content():
    ...
    assert (
        SCRIPTS_SETUP_PATH.exists()
    ), f"Scripts setup doc not found at {SCRIPTS_SETUP_PATH}"
...
class TestRiskMapSetupDoc:
    ...
    def test_file_exists_and_is_non_empty(self, risk_map_setup_content):
        ...
        """
        assert (
            len(risk_map_setup_content) > 0
        ), "Risk map setup doc should not be empty"
        assert (
            len(risk_map_setup_content) > 100
        ), "Risk map setup doc should contain substantial content"
    ...
    def test_references_python_3_14(self, risk_map_setup_content):
        ...
        """
        assert (
            "3.14" in risk_map_setup_content
        ), "Risk map setup doc should reference Python 3.14"
    ...
    def test_does_not_reference_old_python_versions(self, risk_map_setup_content):
        ...
        for pattern in outdated_patterns:
            assert (
                pattern not in risk_map_setup_content
            ), f"Risk map setup doc should not reference '{pattern}'"
    ...
    def test_references_nodejs_22(self, risk_map_setup_content):
        ...
        assert any(
            pattern in risk_map_setup_content for pattern in nodejs_patterns
    ...
    def test_mentions_mise_tool_manager(self, risk_map_setup_content):
        ...
        """
        assert (
            "mise" in risk_map_setup_content.lower()
        ), "Risk map setup doc should mention mise tool manager"
    ...
    def test_mentions_install_deps_script(self, risk_map_setup_content):
        ...
        """
        assert (
            "install-deps" in risk_map_setup_content.lower()
...
class TestScriptsSetupDoc:
    ...
    def test_file_exists_and_is_non_empty(self, scripts_setup_content):
        ...
        """
        assert (
            len(scripts_setup_content) > 0
        ), "Scripts setup doc should not be empty"
        assert (
            len(scripts_setup_content) > 100
        ), "Scripts setup doc should contain substantial content"
    ...
    def test_references_python_3_14(self, scripts_setup_content):
        ...
        """
        assert (
            "3.14" in scripts_setup_content
        ), "Scripts setup doc should reference Python 3.14"
    ...
    def test_does_not_reference_python_3_10(self, scripts_setup_content):
        ...
        for pattern in outdated_patterns:
            assert (
                pattern not in scripts_setup_content
            ), f"Scripts setup doc should not reference '{pattern}'"
    ...
    def test_references_nodejs_22(self, scripts_setup_content):
        ...
        assert any(
            pattern in scripts_setup_content for pattern in nodejs_patterns
...
class TestDocVersionConsistency:
    ...
    def test_both_docs_reference_python_3_14(
        self, risk_map_setup_content, scripts_setup_content
    ):
        """
    ...
    def test_both_docs_reference_nodejs_22(
        self, risk_map_setup_content, scripts_setup_content
    ):
        """
        ...
        risk_map_has_node22 = any(
            pattern in risk_map_setup_content for pattern in nodejs_patterns
        )
        scripts_has_node22 = any(
            pattern in scripts_setup_content for pattern in nodejs_patterns
        )
