...
schema-legal placement. Moving componentFederationProxy out of
componentsExternalTools / componentsToolNetworkControls into
...
_NET_NEW_PLACEMENTS: dict[str, tuple[str, str]] = {
    # Infrastructure -- identity and authorization decision plane
    ...
    "componentSecureLogging": ("componentsInfrastructure", "componentsDeployment"),
    ...
    # External tools -- tool control plane
    "componentFederationProxy": ("componentsExternalTools", "componentsToolNetworkControls"),
    "componentToolNetworkPolicyEnforcementPoint": ("componentsExternalTools", "componentsToolNetworkControls"),
    # External tools -- tool data plane
...
# determined by this layer plus componentTools (recategorized into the tool
# data plane), so an exact-set assertion is safe here in a way it would not
# be for the pre-existing subcategories.
...
class TestAgenticLayerComponentPlacement:
    ...
    def test_component_subcategory(
        ...
        assert subcategory == expected_subcategory, (
            ...
            f"The control-plane / data-plane split is the substance of this layer: a component "
            f"that enforces policy does not belong on the path it guards, and vice versa."
        )
