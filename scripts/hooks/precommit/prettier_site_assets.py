#!/usr/bin/env python3
"""
Pre-commit framework hook that runs prettier on staged site/** assets and
stages the formatted output (Mode B auto-stage).

Invoked by the pre-commit framework with staged filenames as positional argv
(pass_filenames: true). Semantically identical to prettier_yaml.py — only
the `files:` regex in .pre-commit-config.yaml differs.
"""
