...
**Mapping-value purity** (ADR-027 D4c) — checks that every framework mapping value in the four content files round-trips through the generator: values are generated by `framework_mapping_maintainer.py`, not hand-typed. A value that has a version delimiter (`@` or `:`) but fails round-trip is flagged as tampered. Legacy unpinned values (no delimiter) are skipped without error until migration (#343):
...
**Mapping-value drift** (ADR-027 D5/D5a) — Tier-1 drift detection: classifies each mapping value as `skip`, `current`, `valid-but-superseded`, or `invalid` (legacy unpinned and unknown-framework values are `skip`ped until migration in #343; see [hook-validations.md §18](../../scripts/docs/hook-validations.md) for the full state table). Only `invalid` causes a non-zero exit. `valid-but-superseded` values (pinned to a `priorVersions` entry) are reported informationally for maintainer review but do not break CI:
