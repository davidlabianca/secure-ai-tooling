...
- Framework mappings (MITRE ATLAS, NIST AI RMF)
- Lifecycle stages
- Impact types
- Actor access levels
- Additional context
...
- Schema changes (e.g., adding `applicableTo` field to frameworks)
- Framework architecture updates
- Automation/CI/CD improvements
- Tooling enhancements (e.g., template generator)
...
- [ ] Sub-issue: #123 - Template generator implementation
- [ ] Sub-issue: #124 - Pre-commit hook integration

Related Issues:
Parent issue: #104
Sub-issues:
- #123 - Generator implementation
- #124 - Hook integration
```

**Example Walkthrough:**

```yaml
Enhancement Category: Schema Change
Scope: Medium

Rationale & Justification:
Add `applicableTo` field to frameworks schema to enable dynamic framework filtering in templates. Currently, framework applicability is hardcoded.

Implementation Approach:
Phase 1: Update frameworks.schema.json with new field
Phase 2: Add applicableTo arrays to all frameworks in frameworks.yaml
Phase 3: Update validation scripts to check applicability
Phase 4: Update template generator to read from schema

Task List:
- [ ] Update frameworks.schema.json
- [ ] Update frameworks.yaml (4 frameworks)
- [ ] Update validation scripts
- [ ] Update template generator
- [ ] Update documentation

Breaking Changes: Yes
- frameworks.yaml requires new required field
- Validation will fail until all frameworks updated

Migration Plan:
1. Add field to schema with migration grace period
2. Update all framework entries
3. Deploy validation

Testing & Validation Strategy:
- Schema validation passes
- All frameworks have applicableTo arrays
- Validation scripts detect invalid mappings
- Template generator reads applicableTo correctly
```
...
**Controls** use MITRE ATLAS mitigations (AML.M\*) and NIST AI RMF subcategories.

**Risks** use MITRE ATLAS techniques (AML.T\*), STRIDE threat categories, and OWASP LLM vulnerabilities.

**Components and Personas** currently have no framework mappings in practice.

Templates only show applicable frameworks for each entity type.
