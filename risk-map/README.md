...
* **For Human Review (Learn & Assess)**: 
    ...
    * Use the **CoSAI Risk Map Explorer** in [`./site/`](./site/) to match CoSAI-RM personas and browse the linked risks and controls. See [CoSAI Risk Map Explorer](./docs/persona-pages.md) for build and preview instructions.
...
The current GitHub Pages MVP complements that legacy assessment with a persona-first flow that maps users to CoSAI-RM personas, then shows the relevant risks and controls without introducing scoring or maturity grades. See [CoSAI Risk Map Explorer](./docs/persona-pages.md) for the implementation and maintenance workflow.
