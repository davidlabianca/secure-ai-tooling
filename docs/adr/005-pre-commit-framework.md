...
- **Full-tree commands.** Two commands cover two different workflows:
  - `pre-commit run --all-files` runs every hook including generators (Mode B auto-stage). It matches commit-time semantics — generators execute and `git add` their outputs, so this command can mutate the working tree and index. Use it when you want the "what would a commit do?" gate end-to-end.
