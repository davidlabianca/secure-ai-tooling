...
- **Consumer wiring:** `ComponentGraph` fourth-category handling, the `componentsTools` `mermaid-styles.yaml` entry, table/SVG regeneration, and a CI guard that fails on an unstyled category. (Per the erratum above, the persona-ownership item this entry originally listed is withdrawn.)
