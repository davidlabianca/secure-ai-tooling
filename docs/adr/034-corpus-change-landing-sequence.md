...
- **Layer 6 — Modifications to existing risks and controls.** Tuneups and description improvements **not** driven by any new node, plus one-way reference wiring into existing risks and controls that *is* driven by a new node but has no reciprocal back-link to co-locate — for example, adding a newly-landed persona (Layer 2) to an existing risk's or control's `personas` list. Persona references are one-way (Layer 2: personas carry no reciprocal back-link), so this wiring has no Layer 4/5 reciprocal-backlink home; it lands here instead, after the persona it references already exists.
- **Layer 7 — Deprecations.** Unneeded components, controls, risks, **and personas** are retired by a **status flip to `deprecated`, not enum deletion**. The corpus already models this for personas today (per [ADR-021](021-personas-and-self-assessment-schema.md); `personas.schema.json:37-41` defines the `deprecated` boolean). The component, control, and risk schemas do not yet define a `deprecated` field — this layer is forward-looking for those three types until that schema support is added (tracked as follow-up work, see Consequences), not a prerequisite for this ADR. Once available for a type, keeping closed id-enums intact preserves every prior reference and avoids reopening a closed enum. A status flip removes no edges and no references, so it introduces no dangling reference regardless of when it lands; it is placed last so retirement follows every addition and modification that might have kept the entity in use.
...
- **Schema support for `deprecated` status is currently persona-only** (ADR-021). Layer 7 assumes the same status-flip mechanism extends to components, controls, and risks, but those three schemas do not yet define a `deprecated` field — adding it is tracked as follow-up schema work, not a prerequisite for this ADR. ([Review feedback](https://github.com/cosai-oasis/secure-ai-tooling/pull/410#issuecomment-4983458379): the original Layer 7 text incorrectly stated the corpus "already models `deprecated` status for personas and components" — corrected above to persona-only.)
