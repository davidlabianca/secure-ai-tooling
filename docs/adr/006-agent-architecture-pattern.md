...
The CoSAI Risk Map repository delegates increasingly large amounts of work to AI sub-agents: content review, issue-proposal triage, test authoring, implementation, code review, and — as of Phase 2 of the ADR-adoption plan — architectural decision drafting. Each of these agents is defined by a prose specification that names the agent, states its purpose, fixes its input and output contracts, and describes how it composes with other agents.

The pattern emerged implicitly. The first canonical specification was [`scripts/agents/content-reviewer.md`](../../scripts/agents/content-reviewer.md), a 599-line full specification that encodes schema-aware review logic across three modes (`diff`, `full`, `issue`). The second was [`scripts/agents/issue-response-reviewer.md`](../../scripts/agents/issue-response-reviewer.md), a 299-line specification that composes `content-reviewer` in `issue` mode and assembles a contributor-facing review comment. Both were authored as tracked, vendor-neutral prose, with harness-specific files (e.g., `.claude/agents/content-reviewer.md`, the `/draft-issue-comment` skill under `.claude/skills/`) layered on top as thin wrappers for the Claude Code environment the maintainer happens to use.

Phase 2 of the ADR-adoption plan formalized this pattern by adding four more canonical agents in the same directory: [`architect.md`](../../scripts/agents/architect.md) (233 lines), and the contract-only specifications [`testing.md`](../../scripts/agents/testing.md) (91 lines), [`code-reviewer.md`](../../scripts/agents/code-reviewer.md) (94 lines), and [`swe.md`](../../scripts/agents/swe.md) (95 lines). The three implementation-workflow agents previously existed only as Claude Code wrappers under `.claude/agents/`; Phase 2 lifted their vendor-neutral logic out into `scripts/agents/` and reduced the wrappers to thin adapters.
...
Today the pattern is asserted in several places — [ADR-001](001-adopt-adrs.md)'s Context explicitly names "the vendor-neutral canonical agent-architecture pattern in `scripts/agents/`" (line 17); `architect.md`'s Style Conventions section names the wrappers as "implementation details of each environment, not part of the canonical pattern"; the `/draft-issue-comment` skill opens with a "Source of truth" callout deferring to `scripts/agents/issue-response-reviewer.md`. But the pattern itself has no tracked decision record. This ADR is that record.
...
- **Harness wrappers are outside the pattern.** A specific AI environment may layer its own wrappers over the canonical sources — for example, Claude Code uses `.claude/agents/*.md` (YAML frontmatter + the canonical body with `## Agent` and `## Composition` sections stripped, since their content is duplicated in frontmatter) and `.claude/skills/*/SKILL.md` (thin slash-command orchestration that invokes a canonical agent). Wrappers of this kind are **implementation details of the environment that consumes the canonical**, not part of the canonical pattern. Other harnesses (Cursor, Aider, plain CLI) would layer different wrappers over the same `scripts/agents/*.md` sources and would be equally valid consumers.
- **The canonical-to-wrapper transform is shallow.** A wrapper is the canonical source with harness-specific frontmatter prepended and the sections made redundant by that frontmatter removed. It is not a rewrite. Comparing `scripts/agents/content-reviewer.md` with `.claude/agents/content-reviewer.md` shows exactly this: the wrapper adds a YAML block (name, description, tools, model, color) and drops the `## Agent` and `## Composition` sections whose content is already carried by the frontmatter's `name` and `description` fields. Drift beyond this shape is a bug.
- **Authority.** When a canonical and a wrapper disagree, the canonical wins. The `/draft-issue-comment` skill's opening "Source of truth" callout ("When review logic changes, update that file first, then reconcile this skill") is the operating norm: edits flow canonical → wrapper, not the other direction.

## Alternatives Considered

- **Single surface, harness-specific only.** Define agents exclusively as `.claude/agents/*.md` (or the equivalent for whatever single harness the maintainer currently uses), with tool bindings, model identifiers, and harness-idiomatic framings baked in. Rejected: this commits the repository to one AI environment. Future harness adoption would require rewriting every agent rather than wrapping it, and the coalition's vendor-neutral posture (see [ADR-004](004-ai-assistant-trailer.md)) would be contradicted in the agent surface just as it would be in commit trailers.
...
- **Co-equal surfaces with implicit inheritance.** Treat `scripts/agents/*.md` and `.claude/agents/*.md` as parallel definitions, with a convention that they "should stay aligned." Rejected: this was the earlier, looser framing the pattern started with, and it was explicitly rejected on 2026-04-20 during the ADR-001 review. Elevating a specific harness's wrappers to "part of the pattern" reads as prescribing that harness, which the coalition's vendor-neutrality posture does not allow. It also invites drift, because "co-equal" provides no tie-breaking rule when the two files disagree.
...
- Every agent now has two files to keep in sync (canonical + at least one harness wrapper, as long as the maintainer uses a specific harness). Drift between them is a real failure mode. Mitigated by the "canonical wins" rule, by the shallow transform shape, and by the "Source of truth" callouts in wrapper files — but the discipline is ongoing.
...
- Wrappers for harnesses the project does not officially support are not tracked (`.claude/agents/` is maintainer-local via `.git/info/exclude`). A future harness adoption will need its own decision about whether to track wrappers or keep them local.
...
- If the project ever officially supports a second AI harness, the wrapper-tracking question (maintainer-local via `.git/info/exclude`, or tracked in-tree) is a follow-on decision. Not in scope here.
