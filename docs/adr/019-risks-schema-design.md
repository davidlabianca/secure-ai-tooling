...
Phase 0 ([ADR-014](014-yaml-content-security-posture.md)) and Phase 1 Tracks B and C (ADRs [015](015-site-content-sanitization-invariants.md), [016](016-reference-strategy.md), [017](017-yaml-prose-authoring-subset.md)) are accepted. Track A — per-file schema ADRs — runs now. ADR-018 documents `components.schema.json` in parallel; this ADR documents `risks.schema.json`. The two are non-overlapping.

The discovery work that motivated this epic surfaced systemic gaps specific to risks:
...
- **Outbound citations are prose-embedded.** `risks.yaml` carries approximately 55 outbound `<a href="https://...">` tags inside `longDescription` and `examples` prose. None of them are structured; per [ADR-016](016-reference-strategy.md) every one migrates to `externalReferences` during the Phase 2 conformance sweep.
...
| `relevantQuestions` (current, retired) | n/a — ghost field | Removed from schema in Phase 2 sweep (D6) |
...
The Phase 2 conformance sweep tightens `riskmap.schema.json#/definitions/utils/text` (or introduces a more constrained sibling) to match the output shape — `array` of `string | array<string>`, one level of nesting maximum, `array<string>` items must contain at least one element. The tightening is owned by the conformance sweep, not by this ADR; this ADR records the decision that the input/output gap is closed in Phase 2.
...
**Phase 2 sweep tightening.** The conformance sweep adds per-framework ID patterns to the schema. Each canonical framework (MITRE ATLAS, STRIDE, OWASP LLM Top 10, NIST) gets a per-key regex. The patterns live in a new `definitions/framework-mapping-patterns` block inside `frameworks.schema.json` (single-source-of-truth, same pattern as `external-references.schema.json` from [ADR-016](016-reference-strategy.md) D3) and are applied via `propertyNames` + per-property `items.pattern` constraints. The tightening is **schema-only** and does not require validator changes; it is a Phase 2 conformance-sweep deliverable, not a Phase 1 architectural decision. This ADR records the gap and the planned tightening; the exact regexes are picked when the sweep authors the patterns.
...
The field is **retired**. The Phase 2 conformance sweep:
...
The `$ref` integration is a **single-line schema edit** plus the shared-schema authoring (which ADR-016 D3 commits to as a Phase 1 Track A deliverable owned by the conformance sweep). This ADR's scope is the integration declaration; the shared schema's authoring is owned by the sweep.

### D8. Other follow-ups from the discovery report

- **`title` is unbounded `string`.** No `maxLength`, no `pattern`, no required-prefix rule. In practice every risk title is 3-7 words, but a maliciously long title would pass schema validation and break Mermaid layout. The Phase 2 sweep adds `"maxLength": 120` (chosen to accommodate the longest current title — `Federated/Distributed Training Privacy` plus headroom).
...
| D1 prose fields conform to `definitions/utils/text` shape | schema `$ref` | Machine-enforced (existing); tightened in Phase 2 sweep (D4) |
| D1 prose content conforms to ADR-017 subset | `validate-yaml-prose-subset` lint | Machine-enforced (Phase 2 sweep, ADR-017) |
...
| D4 prose nesting depth ≤ 1 | output schema (`persona-site-data.schema.json`); input tightened in Phase 2 sweep | Partially machine-enforced (output); input tightened in sweep |
...
| D5 per-framework mapping ID regex | not enforced today; added in Phase 2 sweep | Deferred to sweep (D5) |
| D5 per-mapping rationale fields | not added | Decided against (D5) |
| D6 `relevantQuestions` retirement | schema removal + YAML cleanup | Phase 2 sweep deliverable (D6) |
| D7 `externalReferences` shape | shared schema `external-references.schema.json` `$ref` | Machine-enforced (Phase 2 sweep, ADR-016) |
| D7 `externalReferences` content (sentinel resolution, URL shape, type enum) | shared schema + `validate_prose_references.py` | Machine-enforced (Phase 2 sweep, ADR-016) |
| D8 `title` `maxLength: 120` | schema constraint | Phase 2 sweep deliverable (D8) |
| D8 `additionalProperties: false` on `risk` | schema constraint | Phase 2 sweep deliverable (D8) |
| D8 `tourContent` required | not enforced; framework-content design question | Prose-only guidance |

Every content rule is machine-enforced or has a named Phase 2 mechanism. The only "prose-only guidance" row is `tourContent` requiredness, which is deliberately a framework-content design question owned by `risk-map/docs/design/` rather than the schema.
...
- **Risks schema is now documented as a unit.** Future readers find every field's class, every enforcement layer, every gap, and every Phase 2 deliverable in one place. The 28-risk-id enum, the five-category enum, the four `oneOf`-shaped taxonomy enums, and the open `mappings` object are no longer institutional knowledge.
...
- **Per-framework mapping-ID regex is deferred.** The Phase 2 sweep authors the patterns; until that lands, `AML.T0020` and `AML T0020` and `aml-t0020` continue to pass schema validation. Content-reviewer is the human enforcement layer in the meantime.
...
- **Phase 2 conformance sweep — risks-specific deliverables.** A coordinated commit (or sequence) that:
...
- **Sibling ADRs.** [ADR-018](018-components-schema-design.md) is the parallel per-file schema ADR for components, running concurrently with this one. Pending per-file ADRs for `controls.schema.json` and `personas.schema.json` follow the same shape; each decides independently on the ADR-017 D3 opt-in question.
