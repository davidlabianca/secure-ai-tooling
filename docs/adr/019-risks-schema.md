...
`risk-map/schemas/risks.schema.json` is the JSON Schema (Draft-07) that governs `risk-map/yaml/risks.yaml`, the largest and most-cited content surface in the framework. Risks are read by every downstream consumer the framework owns: `scripts/hooks/yaml_to_markdown.py` (table generation under `risk-map/tables/`) and `scripts/build_persona_site_data.py` (the persona-site JSON contract codified in [ADR-011](011-persona-site-data-schema-contract.md)). External redistributors per [ADR-014](014-yaml-content-security-posture.md) P1 also read the YAML directly. (`scripts/hooks/riskmap_validator/graphing/` no longer consumes `risks.yaml`: `RiskNode` and `parse_risks_yaml` were removed when [#477](https://github.com/cosai-oasis/secure-ai-tooling/issues/477) deleted the risk graph generator.)
