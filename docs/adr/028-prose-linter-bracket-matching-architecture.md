...
3. **Greedy / non-greedy specification.** Bold non-greedy on close; italic non-greedy on close — asterisk italic is intraword-permissive, underscore italic qualifies as a delimiter iff it is **not intraword** (at least one immediate side is whitespace, punctuation, or a string boundary per ADR-017 D1), so an intraword `\S_\S` underscore where both neighbors are word characters (snake_case) does not qualify and the run consumes as TEXT. Once a `_` qualifies, `_classify_emphasis_shape(span, "_")` derives `complete`/`open`/`close` from interior edge whitespace exactly as it does for `*`/`**`; underscore italic therefore carries `open`/`close` shapes like asterisk, so a nested underscore span (`_foo _nested_ bar_`) splits into open/text/close tokens and is caught by the D5 depth walk. The 2026-06-04 addendum below records the flip from the prior whitespace-flanked-only rule. URL regexes greedy with bracketed exclusion `[^\s{]+` so a URL adjacent to a `{{` sentinel terminates at the brace boundary; fenced code non-greedy across newlines with `re.DOTALL`; sentinel scan brace-depth-aware so `{{id{{ref:x}}}}` consumes as one `INVALID_SENTINEL` token. Unclosed `{{` returns `None` from `_match_sentinel`; the caller emits the remainder as `TEXT`.
...
Edge cases the tokenizer does not emit as emphasis at all carry `shape="neutral"` like any other TEXT: `**` alone; `**foo` unclosed; `**` paired with no inner character; `****` consumed as two TEXT runs because `(.+?)` requires at least one inner character; `__bold__` consumed as TEXT per ADR-017 D1's asterisk-only-bold stance; and **intraword underscore runs like `home_bar and foo_baz` consumed as TEXT** because each underscore is intraword (both immediate neighbors are word characters), so it does not qualify as a delimiter per the not-intraword flanking rule — a snake_case identifier pair is not an italic span.
...
**Addendum (2026-06-04) — D-Open-21.2 flip (a)→(b).** This responds to the PR #367 review (@shrey-bagga, CHANGES_REQUESTED, 2026-05-30, maintainer-authorized fix). D-Open-21 tightened the underscore-italic regex to fix the snake_case intraword false positive, and its sub-decision D-Open-21.2 chose form (a) the "cosai-simpler" whitespace-or-boundary flanking over (b) fuller CommonMark-style flanking, with an explicit "amend later" deferral if a punctuation-adjacent corpus case surfaced. The review found that form (a) introduced two real gaps: (1) `_foo_,` — italic immediately followed by a comma — tokenized as TEXT because the close rule rejected a punctuation-flanked close; and (2) `_foo _nested_ bar_` tokenized as one `complete` ITALIC with no `nested emphasis` diagnostic, because the over-restrictive close rule blocked the open/close split that drives nested detection, so nested underscore escaped ADR-017 D1 even though the asterisk equivalent `*foo *nested* bar*` is caught.

We flip D-Open-21.2 (a)→(b) and adopt fuller CommonMark-style `_` flanking: a `_` qualifies as an emphasis delimiter iff it is **not intraword** — at least one immediate side is whitespace, punctuation, or a string boundary. Intraword (both immediate neighbors are word characters — letters/digits, excluding `_`) never delimits, preserving the snake_case fix. Once a `_` qualifies, `_classify_emphasis_shape(span, "_")` derives `complete`/`open`/`close` from interior edge whitespace exactly as it already does for `*`/`**`. This single change resolves both gaps and the snake_case requirement together. Target behavior, empirically confirmed against the production tokenizer with the new regex:

- `home_bar and foo_baz` → **TEXT** (both sides word chars → intraword; snake_case fix preserved).
- `_foo_,` → **ITALIC** `complete` (close `_` flanked by the punctuation `,`).
- `_foo _nested_ bar_` → splits into `[ITALIC "_foo _" shape=open, TEXT "nested", ITALIC "_ bar_" shape=close]`; the D5 depth walk emits the `nested emphasis` diagnostic, parallel to the asterisk case.
- `_italic_` and `_See important, then act._` → still **ITALIC** `complete`.

Consequence to make explicit: underscore italic is **no longer always `complete`** — it can now emit `open` and `close` shapes, which is exactly what enables nested-underscore detection. D3 invariant 3 and the D3 shape table prose are reconciled accordingly.

Corpus-safety gate: the fuller-flanking regex was re-run over all 569 prose fields in the four content YAMLs (`risks.yaml`, `controls.yaml`, `components.yaml`, `personas.yaml`) — the Spike S3 re-run — and produced **zero** tokenization changes versus the shipping cosai-simpler regex. snake_case stays TEXT; no surprise italics. The flip is corpus-safe.

This is the "amend later" trigger that D-Open-21.2's own lock rationale anticipated, so no new lock is created and **no other §8 lock changes**. Status remains **Accepted**.
...
- **The underscore-italic tightening (D3 invariant 3) is content-visible.** Snake_case identifier pairs like `home_bar and foo_baz` no longer tokenize as italic — a corpus-observable shift even though no new rejection is added. The corpus impact probe (Follow-up below) runs before Status flips from Draft to Accepted; if surprises surface, the rule is revisited. The 2026-06-04 addendum (D5 section) flips the underscore delimiter rule from whitespace-flanked-only to not-intraword (fuller CommonMark-style); the snake_case-to-TEXT behavior described here is unchanged by the flip and the re-run probe reported zero corpus changes.
